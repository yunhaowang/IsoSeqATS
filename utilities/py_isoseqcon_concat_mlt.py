...
def output_gpd(dic_chr_strand_en,concatenate_gpd):
	for chr_strand_en in dic_chr_strand_en.keys():
		...
		for junction_info in dic_chr_strand_en[chr_strand_en].keys():
			...
			print >>concatenate_gpd, "\t".join([gene_id,isoform_id,chr,strand,tss,tts,gene_count,isoform_count,en,exon_start,exon_end])
...
10. exon start set
11. exon end set'''
	parser = argparse.ArgumentParser(description="Function: concatenate multi-exon isoforms and/or reads if they have the same junction combination set.",formatter_class=argparse.ArgumentDefaultsHelpFormatter)
